...
export class Enemy {
    ...
    constructor(game, x, y, anim, animLength) {
        ...
        var xDestination = this.game.rnd.integerInRange(100, this.game.world.width / 2);
        var rndInt = this.game.rnd.integerInRange(0, this.game.audio.ufoSnd.length - 1);
        this.game.audio.ufoSnd[rndInt].play();
        var ufo = this.game.add.sprite(xDestination - this.game.rnd.integerInRange(0, xDestination), 0, 'ufo');
            ...
            if (_this.enemy.ufo_exists) {
                if (_this.enemy.ufo.x > _this.enemy.drop_enemy_at_x) {
                    ...
                    this.game.time.events.add(Phaser.Timer.SECOND * 3, function () {
                        var beamTween = _this.game.add.tween(_this.enemy.ufo_beam)
                        ...
                        var ufo_tween = _this.game.add.tween(_this.enemy.ufo);
                        ufo_tween.to({
        ...
        enemy.update = function () {
            if (enemy) {
                ...
                if (bar.barSprite) {
                    ...
                    var y = enemy.y > this.game.height - enemy.height
                        ? enemy.y + 20
                        : enemy.y + 30;
    ...
    fire(enemy) {
        if (enemy.alive && enemy.game.time.now > enemy.fireLastTime) {
            ...
            var bullet = new Bullet(enemy.game, enemy.x, enemy.y, true);
    ...
    explode(enemy) {
        if (enemy.alive) {
            ...
            var explode = enemy.game.add.sprite(enemy.x - 100, enemy.y - 150, 'explode', 19);
    ...
    showStealingSign(enemy) {
        ...
        var stealingDirection = enemy.x < enemy.mainState.towers.children[0].x ? 'left' : 'right';
